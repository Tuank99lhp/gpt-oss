...
// X[b*N + i] = alpha[b] * Y[b*N + i]
__global__ void k_xpy_batch_alpha_vec(float * __restrict__ X,
                                       const float * __restrict__ Y,
                                       const float * __restrict__ alpha_b,
                                       int N, int B) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
...
static inline void xpy_gpu_batch_alpha_vec(float *X, const float *Y,
                                            const float *alpha_b, // [B]
                                            int N, int B,
                                            hipStream_t stream = 0) {
  const int BSx = 256;
  dim3 block(BSx, 1, 1);
  dim3 grid((N + BSx - 1) / BSx, B, 1);
  hipLaunchKernelGGL(k_xpy_batch_alpha_vec, grid, block, 0, stream,
                     X, Y, alpha_b, N, B);
}