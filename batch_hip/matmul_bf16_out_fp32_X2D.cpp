...
//       if (br < B && (kk + 3) < K) {
//         const hip_bfloat16* g = X[br] + kk;     // X is now hip_bfloat16**
...
//   hipLaunchKernelGGL(
//     HIP_KERNEL_NAME(k_gemm_bf16core_yfp32_2x2waves_X2D<BM, BN, BK, WAVES_M, WAVES_N, APAD, BPAD>),
//     grid, block, 0, stream,
//     X, W, Y, K, M, B
//   );
// }
  ...
  for (int k0 = 0; k0 < K; k0 += BK) {
    ...
    for (int t = 0; t < (BK / 16); ++t) {
      ...
      if (brow_base < B && base_abs < K) {
        ...
        const hip_bfloat16* gA = X[brow_base] + base_abs;
        if (valid == 4) {
