...
__global__ void k_rmsnorm_batch_bf16(hip_bfloat16 * __restrict__ out,     // [B, H]
                                const float * __restrict__ x, // [B, H]
                                const float * __restrict__ w, // [H] (shared cho mọi b)
                                int H, float eps) {
  extern __shared__ double ssum[]; // size = blockDim.x
  ...
  // 3) tính inv_rms và ghi kết quả (theo stride)
  const float inv_rms = rsqrtf((float)(ssum[0] / (double)H + (double)eps));
  hip_bfloat16* ob = out + b * H;
  for (int i = tid; i < H; i += blockDim.x) {
    ob[i] = hip_bfloat16(w[i] * (xb[i] * inv_rms));
  }
}

// Wrapper tiện dụng
static inline void rmsnorm_batch_gpu_bf16(hip_bfloat16 *out, const float *x,
                                     const float *weight,
                                     int B, int H, float eps = 1e-5f,
                                     hipStream_t stream = 0) {
  // chọn block size hợp lý; stride loop sẽ cover H
  const int BS = (H >= 1024) ? 1024 : (H >= 512 ? 512 : 256);
  size_t shmem = BS * sizeof(double);
  hipLaunchKernelGGL(k_rmsnorm_batch_bf16, dim3(B), dim3(BS), shmem, stream,
                     out, x, weight, H, eps);
}